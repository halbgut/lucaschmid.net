...
/***/ function(module, exports, __webpack_require__) {
	...
	riot.tag2('scrollthingy', '<yield></yield>', 'scrollthingy,[riot-tag="scrollthingy"] { position: relative; display: block; } scrollthingy > section,[riot-tag="scrollthingy"] > section { min-height: 100vh; min-width: 100%; background-color: white; } scrollthingy > section:nth-child(1),[riot-tag="scrollthingy"] > section:nth-child(1) { background-color: hsl(0, 40%, 80%); } scrollthingy > section:nth-child(2),[riot-tag="scrollthingy"] > section:nth-child(2) { background-color: hsl(20, 40%, 80%); }', '', function (opts) {
	  var _this = this;

	  var updatePosition = function updatePosition(factor, parentHeight, element, nth, lastPos) {
	    var pos = window.scrollY / factor * -1 + 100 * nth;
	    if (pos < 0) element.style.transform = 'translateY(' + pos + 'vh)';else pos = 0;
	    if (pos >= 0 && lastPos !== 0) element.style.transform = 'translateY(0vh)';
	    window.requestAnimationFrame(updatePosition.bind(_this, factor, parentHeight, element, nth, pos));
	  };

	  this.on('mount', function () {
	    var height = _this.root.clientHeight;
	    var children = Array.from(_this.root.children);
	    var factor = (height - window.innerHeight) / 100;
	    ...
	    window.requestAnimationFrame(function () {
	      return children.forEach(updatePosition.bind(_this, factor, height));
	    });
	  });
	});
	/* WEBPACK VAR INJECTION */}.call(exports, __webpack_require__(1)))
